...
// Function to save particle positions to a CSV file
void saveParticlePositionsToCSV(const std::vector<Particle>& particles, const std::string& filename, int iteration) {
    ...
    // Write iteration number (optional, for tracking)
...
void runPSO(std::vector<Particle>& particles, std::vector<double>& globalBestPosition, double& globalBestFitness) {
    ...
    for (int iter = 0; iter < MAX_ITERATIONS; iter++) {
        ...
        saveParticlePositionsToCSV(particles, "particle_positions.csv", iter);
    }
