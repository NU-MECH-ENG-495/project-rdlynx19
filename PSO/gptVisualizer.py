...
# Read particle positions and global best data from CSV
def read_particle_positions(filename):
    ...
    return np.array(positions), np.array(global_best_positions), np.array(global_best_fitness)

# Animation function
def update(frame, positions, global_best_positions, scat, best_scat, fitness_line, global_best_fitness):
    ...
    return scat, best_scat, fitness_line

# Main function
def visualize_particle_positions(filename):
    # Read data
    positions, global_best_positions, global_best_fitness = read_particle_positions(filename)
    ...
    # Set up scatter plot
    ax1.set_xlim(np.min(positions[:, :, 0]) - 1, np.max(positions[:, :, 0]) + 1)
    ...
    ani = animation.FuncAnimation(
        ...
        fargs=(positions, global_best_positions, scat, best_scat, fitness_line, global_best_fitness),
